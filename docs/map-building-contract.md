...
- **Openings are placed by anchor**: `near: {x, y}` and the compiler snaps the opening onto the nearest point of the wall. Do not compute arc length. `door`, `rollup` and `archway` cut the wall into a genuine absence of collision; `window` glazes it without cutting. The authored `width` is the *clear* opening. A passable opening is never compiled below `2 × (bot radius + steering margin)` (currently `2 × (24 + 8) = 64`), and the compiler pulls each surviving wall spine back by half that wall's own thickness so capsule caps do not take any of that clearance. The compiled doorway width, visible threshold, collision gap, audits and navigation all use the resulting clear width.
...
- **Default to solid.** An object a person would walk around is a collider — a chair, a plant, a sink, a bin. Passability is the rare, deliberate exception, not the fallback for anything small. Every ghost is the map lying to the player about what is cover, and the lie is worst exactly where it matters: in a doorway, on an entrance approach, on the only route through a room.
- **A passable object is see-through, not faintly outlined.** A lighter edge does not read at the distance the camera actually sits at — the player finds out by walking into the one thing that stopped them. If an object can be walked through, the floor must be visible through it, and it draws no lift and no cast shadow. Solid is opaque. Reserve this for things a floor genuinely shows through: coverings, decals, kerbs, spills.
